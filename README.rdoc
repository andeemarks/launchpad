...
* Due to limitations in the Portmidi gem where it's not possible to specify which channel is used to send MIDI messages, all comms with the Launchpad is done via the default channel 1.  This means some workarounds have been needed to access functionality like flash and pulse, which usually require channel 2 messages.  All of these workarounds have been implemented via sending System Exclusive messages as per the above reference guide.
